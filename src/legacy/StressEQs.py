...
def getStress(interior, e_in, colat, lon, steps, this_step, oblq, phase, NSRdelta):
    ...
    #pi = 3.14159265359
    kyear = 1./3.156E10 # kyear in sec (inverted to ease convertion of s_j from inverse kyears to sec
    year2sec = 3600.*24.*365.  # conversion for NSR sj values
    
    periodInSec = 306000.0 # sec;
    n = 2.*np.pi/periodInSec # [rad/sec]
    #t = orbitFrac*periodInSec
    t = (this_step/steps)*periodInSec
    visc = 1E21          # surface, Pa*s
    ...
    sigTheta   = myStressThThTot*(np.square(np.cos(zeta)))+myStressPhPhTot*(np.square(np.sin(zeta)))+myStressThPhTot*np.sin(2.*zeta)  # Corresponds to Terry's sigma 1
    sigPhi = myStressThThTot*(np.square(np.sin(zeta)))+myStressPhPhTot*(np.square(np.cos(zeta)))-myStressThPhTot*np.sin(2.*zeta)      # Corresponds to Terry's sigma 2
    ...
    if (heading >= (2.*np.pi)):       # Making sure azimuth values fall between 0 and 360. WOULD LOVE TO CHANGE THIS TO MOD AND INCORPORATE ABOVE.
